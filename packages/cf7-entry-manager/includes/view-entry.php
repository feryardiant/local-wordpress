		...
		->div( array( 'id' => 'poststuff', ), static fn ( $elm ) => $elm
			->div( array( 'id' => 'post-body', 'class' => 'metabox-holder columns-2 wp-clearfix' ),
				...
				->div( array( 'id' => 'postbox-container-1', 'class' => 'postbox-container' ),
					static fn ( $elm ) => $elm
					->section( array( 'id' => 'cf7em-info', 'class' => 'cf7em-box postbox' ),
						static fn ( $elm ) => $elm
						...
						->div( array( 'class' => 'inside' ),
							static fn ( $elm ) => $elm
							->div( array( 'class' => 'cf7em-row cf7em-info' ),
								static fn ( $elm ) => $elm
								->div( array( 'class' => 'cf7em-col cf7em-info-field' ),
									static fn ( $elm ) => $elm
									->p( child: __( 'Submitted', 'cf7-entry-manager' ) )
								)
								->div( array( 'class' => 'cf7em-col cf7em-info-value' ),
									static fn ( $elm ) => $elm
							...
							->div( array( 'class' => 'cf7em-row cf7em-info' ),
								...
								->div( array( 'class' => 'cf7em-col cf7em-info-value ' . ( $item->form_id ? '' : 'cf7em-no-value' ) ),
									static fn ( $elm ) => $elm
							...
							->div( array( 'class' => 'cf7em-row cf7em-info' ),
								...
								->div( array( 'class' => 'cf7em-col cf7em-info-value ' . ( $item->author_id ? '' : 'cf7em-no-value' ) ),
									static fn ( $elm ) => $elm
									->p( array( 'class' => $item->author_name ? '' : 'cf7em-no-value' ),
									...
									->p( array( 'class' => $item->author_email ? '' : 'cf7em-no-value' ),
										$item->author_email ? esc_html( $item->author_email ) : sprintf(
											'<span aria-hidden="true">—</span><span class="screen-reader-text">(%s)</span>',
											__( 'no email info', 'cf7-entry-manager' )
										)
									)
									->p( array( 'class' => $item->author_phone ? '' : 'cf7em-no-value' ),
				...
				->div( array( 'id' => 'postbox-container-2', 'class' => 'postbox-container' ),
					static fn ( $elm ) => $elm
					->section( array( 'id' => 'cf7em-entry', 'class' => 'cf7em-box postbox', ),
						static fn ( $elm ) => $elm
						->header( array( 'class' => 'postbox-header' ),
							static fn ( $elm ) => $elm
							...
							static function ( $elm ) use ( $item ) {
								foreach ( $item->form()->scan_form_tags() as $tag ) {
									...
									$elm->div( array(
										...
										->div( array( 'class' => 'cf7em-col cf7em-submission-field' ),
											static fn ( $elm ) => $elm->p( child: esc_html( $tag->name ) )
										)
										->div( array( 'class' => "cf7em-col cf7em-submission-value cf7em-type-{$tag->basetype}", ),
											static fn ( $elm ) => match ( $tag->basetype ) {
												...
												default => $elm->p( child: esc_html( $has_value ? $value : __( 'No answer', 'cf7-entry-manager' ) ) ),
											}
										)
										->div( array( 'class' => 'cf7em-col cf7em-submission-info' ),
											static fn ( $elm ) => $elm
											->when( ! empty( $tag->options ), static fn ( $elm ) => $elm
												->span( array( 'class' => 'cf7em-submission-option' ),
